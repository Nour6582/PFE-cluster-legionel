...
class LoginViewS(LoginView):
    #print("um mail est envoyer")
    def get_success_url_allowed_hosts(self):
        print("um mail est envoyer")
        if  self.request.user.id :
            wilayas=self.request.user.Wilaya
            Mailfrom=self.request.user.email
            patient=identitePatient.objects.all()
            n=0
            for i in patient:
                if i.ResidencePermanent.adresse.wilaya==wilayas and i.casmalade=='Confirmé':
                    n+=1

            message="il y'a "+str(n)+" de cluster legionella dans la wilaya   "+ wilayas        
            send_mail(
            'Alerte cluster',
             message,
            Mailfrom,
            ['nourairdrop@gmail.com'],
            fail_silently=False,)
...
#        return identitePatient.objects.filter(userprofile=userprofile) 
     ...
     #Si Thérapie immunosuppressive en cours, préciser
        ...
        "form":createform()
            ...
            return render(request,"formulaire1.html",context)  
