...
class DetailPatient(LoginRequiredMixin,DetailView):
    template_name="Details_patient.html"
...
def InfoRadiologiqueform(request,pk):
    ...
    title="Information Radiologique"
    if request.method=="POST":
...
def QuestionAvecPrelevementform(request,pk):
    ...
    title="Question Avec Prelevement"
    if request.method=="POST":
...
def casMaladeform(request,pk):
    form=casMaladeForm()
    title="Definir le profile du cas du patient"
    if request.method=="POST":
